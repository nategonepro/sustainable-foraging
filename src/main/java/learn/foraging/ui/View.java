...
public class View {
    ...
    public String getForagerState(){
        String abbr="";
        do {
            abbr = io.readRequiredString("Input state abbreviation [XX]: ");
            if(abbr.length() != 2){
                io.println("State abbreviation must be 2 characters in format [XX]");
            }
        }while(abbr.length() != 2);
        return abbr;
    }
    ...
    public Forager makeForager(){
        ...
        forager.setFirstName(io.readRequiredString("Enter forager first name: "));
        forager.setLastName(io.readRequiredString("Enter forager last name: "));
        forager.setState(getForagerState());
        return forager;
    }
    ...
    public void displayForagers(List<Forager> foragers, String state){
        if(foragers.size()==0){
            io.println("No foragers from " + state + " found");
        }

        for(Forager forager : foragers){
            io.printf("%s %s%n", forager.getFirstName(), forager.getLastName());
        }
    }

    public void displayForageTotals(HashMap<Item, Double> totals, LocalDate forageDate){
        io.println("Forage totals for " + forageDate);
    ...
    public void displayCategoryTotals(HashMap<Category, BigDecimal> totals, LocalDate forageDate){
        io.println("Category totals for " + forageDate);
