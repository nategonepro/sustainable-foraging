...
class ItemFileRepositoryTest {
    ...
    void shouldNotFindMissing(){
        Item beepleBopple = repository.findById(20000);
        assertNull(beepleBopple);
    }
    ...
    void shouldUpdate() throws DataException {
        Item item = new Item();
        item.setId(1);
        item.setName("Duckweed");
        item.setCategory(Category.INEDIBLE);
        item.setDollarPerKilogram(new BigDecimal(100));
    ...
    void shouldNotUpdateMissing() throws DataException {
        Item item = new Item();
        item.setId(30);
        item.setName("Nonexistent Shrubbery");
        item.setCategory(Category.MEDICINAL);
        item.setDollarPerKilogram(new BigDecimal(1000.01));
